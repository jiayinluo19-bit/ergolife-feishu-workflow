...
def _current_open_id(request: Request) -> str | None:
    """Trust only the signed Feishu session for employee identity."""
    return feishu_identity.read_session(request.cookies.get("ergolife_session"))
...
def dashboard(
    ...
    try:
        data = runtime.product_dashboard_data(view=view, open_id=open_id)
    except ValueError as exc:
        return HTMLResponse(f"<h1>请求有误</h1><p>{html.escape(str(exc))}</p>", status_code=400)
    return HTMLResponse(render_product_workbench(data, view, os.getenv("FEISHU_APP_ID", "").strip()))
